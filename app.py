...
def cargar_datos(file_path):
    ...
    try:
        ...
        if df_merged.empty:
            print("ALERTA: El merge entre la lista de entrenadores y los datos de Pokémon no produjo resultados. Verifique que los nombres de entrenadores coincidan en el archivo Excel.")
            return pd.DataFrame()
        ...
        # Corrección clave: Asegurarse de que la columna a renombrar ('Tipo Entrenador_x') existe
        if 'Tipo Entrenador_x' in df_merged.columns:
            ...
            print("ALERTA: La columna 'Tipo Entrenador_x' no se encontró después del merge. Retornando DataFrame vacío.")
            return pd.DataFrame()
...
if df_final.empty:
    print("ADVERTENCIA: La aplicación se está iniciando con un DataFrame vacío. La búsqueda no arrojará resultados.")
...
def index():
    ...
    # Asegurarse de que el DataFrame no esté vacío antes de filtrar
    if request.method == 'POST' and not df_final.empty:
        ...
        # Usar el nombre de columna correcto y verificado: 'Tipo Entrenador'
...
if __name__ == '__main__':
    # Usar el puerto definido por el entorno o 5000 por defecto
