...
def cargar_datos(file_path):
    ...
    try:
        ...
        if not all_pokemon_data: return pd.DataFrame()
...
# --- IMPRESIÓN DE DATOS DISPONIBLES (AL INICIAR) ---
if not df_final.empty:
    print("¡Éxito! Los datos se cargaron correctamente.")
    print("--- MUESTRA DE DATOS DISPONIBLES ---")
    print(f"Tipos de Entrenador: {df_final['Tipo Entrenador'].unique().tolist()}")
    # Mostramos solo los primeros 15 para no saturar el log
    print(f"Algunos Nombres de Pokémon: {df_final['Nombre Pokémon'].unique().tolist()[:15]}")
    print("-------------------------------------")
else:
...
def index():
    ...
    if request.method == 'POST' and not df_final.empty:
        ...
        tipo_entrenador = request.form.get('tipo_entrenador', '').strip()
        modalidad = request.form.get('modalidad', '').strip()
        nombre_pokemon = request.form.get('nombre_pokemon', '').strip()

        # --- IMPRESIÓN DE TÉRMINOS DE BÚSQUEDA ---
        print(f"--- BÚSQUEDA REALIZADA ---")
        print(f"Términos recibidos: Tipo='{tipo_entrenador}', Nombre='{nombre_entrenador}', Modalidad='{modalidad}', Pokémon='{nombre_pokemon}'")
